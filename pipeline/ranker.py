...
def score(features: Dict, semantic_sim: float, graph_boost: float = 0.0) -> float:
    raw = (
        ...
        + min(
            100.0,
            features.get("applied_ml_years", 0.0) / 6.0 * 100
        ) * WEIGHTS["applied_ml_years"]
    ...
    applied_ml_years = features.get(
        "applied_ml_years",
        0.0
    )

    if applied_ml_years >= 5:
        seniority_bonus += 2.0

    if applied_ml_years >= 7:
        seniority_bonus += 2.0

    if features["skill_match"] >= 80:
        seniority_bonus += 1.5

    if features.get("shipped_system", 0.0) >= 75:
        seniority_bonus += 2.0

    if features.get("product_impact", 0.0) >= 70:
        seniority_bonus += 1.5

    fit_penalty = features.get(
        "fit_penalty",
        0.0
    )
